    ...
    onTokenReceived: (token) => {
      ...
      if (isClientForwardedTokenMode(getEffectiveAuthType())) {
        ...
        NotificationsManager.success(
          "Token held for this browser session. Tools can now be loaded and configured; the token is not saved to aiOS.",
        );
        return;
      }
              ...
              <>
                ...
                {!oauthFlowTypeValue && !isDelegateAuth && (
                  <Alert
                    ...
                    description="Choose Machine-to-Machine (M2M) or Interactive (PKCE) so aiOS authenticates it the way you intend, then save. Until it is set, aiOS falls back to interactive per-user auth and treats a machine-to-machine credential shape conservatively."
                  />
                )}
                <Form.Item
                  label={
              ...
              <>
                ...
                <Form.Item
                  label={
                    <span className="text-sm font-medium text-gray-700 flex items-center">
                      AWS Role ARN
                      <Tooltip title="Optional. IAM role ARN to assume via STS before signing. If set, aiOS calls sts:AssumeRole to get temporary credentials.">
                        <InfoCircleOutlined className="ml-2 text-blue-400 hover:text-blue-600 cursor-help" />
                      </Tooltip>
                    </span>
                  }
